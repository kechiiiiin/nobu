...
- 状態は **気になる／買った／読んでる／保留／読了** の5つ。どこからどこへでも切り替えられ、変えるたびに `book_event` に1行残る（取り消しの土台）。**保留は読書の回を開いたまま残す**（中断中。もう一度「読んでる」にすれば同じ回の続き）
- 回の中で**実際に読んだ日**は `reading_day`（本×日で1行・UNIQUE）。「読んでる」「読了」に切り替えた日は自動で入り、その切り替えを取り消すと一緒に消える（`created_event_id` が印。手で押した日は NULL なので残る）
